...
public class SpacechanChanPerformer extends ChanPerformer
{
	@Override
	public ReadThreadsResult onReadThreads(ReadThreadsData data) throws HttpException, InvalidResponseException
	{
		SpacechanChanLocator locator = SpacechanChanLocator.get(this);
		...
		if (jsonObject != null && data.pageNumber >= 0)
		{
			try
			{
				...
				{
					threads[i] = SpacechanModelMapper.createThread(threadsArray.getJSONObject(i),
							locator,false);
				}
		...
		else if (jsonArray != null)
		{
			if (data.isCatalog())
			{
				try
				{
					...
					{
						threads.add(SpacechanModelMapper.createThread(jsonArray.getJSONObject(i),
								locator, true));
					}
					return new ReadThreadsResult(threads);
				}
	...
	public ReadPostsResult onReadPosts(ReadPostsData data) throws HttpException, InvalidResponseException
	{
		...
		if (jsonObject != null)
		{
			try
			{
				return new ReadPostsResult(SpacechanModelMapper.createThread(jsonObject, locator, false));
			}
			catch (JSONException e)
			{
				throw new InvalidResponseException(e);
	...
	public ReadBoardsResult onReadBoards(ReadBoardsData data) throws HttpException, InvalidResponseException
	{
		...
		try
		{
			return new ReadBoardsResult(new SpacechanBoardsParser(responseText).convert());
		}
