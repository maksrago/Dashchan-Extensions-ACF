package com.sixsixthree.dashchan.chan.xchan;
...
public class XchanChanPerformer extends ChanPerformer
{
	@Override
	public ReadThreadsResult onReadThreads(ReadThreadsData data) throws HttpException, InvalidResponseException
	{
		XchanChanLocator locator = XchanChanLocator.get(this);
		...
		if (jsonObject != null && data.pageNumber >= 0)
		{
			try
			{
				...
				{
					threads[i] = XchanModelMapper.createThread(threadsArray.getJSONObject(i),
							locator,false);
				}
		...
		else if (jsonArray != null)
		{
			if (data.isCatalog())
			{
				try
				{
					...
					{
						threads.add(XchanModelMapper.createThread(jsonArray.getJSONObject(i),
								locator, true));
					}
					return new ReadThreadsResult(threads);
				}
	...
	public ReadPostsResult onReadPosts(ReadPostsData data) throws HttpException, InvalidResponseException
	{
		...
		if (jsonObject != null)
		{
			try
			{
				return new ReadPostsResult(XchanModelMapper.createThread(jsonObject, locator, false));
			}
			catch (JSONException e)
			{
				throw new InvalidResponseException(e);
	...
	public ReadBoardsResult onReadBoards(ReadBoardsData data) throws HttpException, InvalidResponseException
	{
		...
		try
		{
			return new ReadBoardsResult(new XchanBoardsParser(responseText).convert());
		}
